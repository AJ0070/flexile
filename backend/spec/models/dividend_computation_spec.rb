...
RSpec.describe DividendComputation do
  ...
  describe "validations" do
    ...
    it { is_expected.to validate_inclusion_of(:ready_for_payment).in_array([true, false]) }
    
    it "validates that dividends_issuance_date is not in the past on create" do
      computation = build(:dividend_computation, dividends_issuance_date: 1.day.ago)
      expect(computation).not_to be_valid
      expect(computation.errors[:dividends_issuance_date]).to include("can't be in the past")
    end
  end
  ...
  describe "scopes" do
    # Create a separate company for these tests to ensure isolation
    let(:test_company) { create(:company) }
    
    # Create test computations in a before block with the test company
    before do
      @pending_computation = create(:dividend_computation, company: test_company, ready_for_payment: false)
      @ready_computation = create(:dividend_computation, company: test_company, ready_for_payment: true, dividends_issuance_date: Date.current)
      @future_computation = create(:dividend_computation, company: test_company, ready_for_payment: true, dividends_issuance_date: 1.day.from_now)
      
      # Ensure the main test computation is not affecting our tests
      if @dividend_computation
        @dividend_computation.update(company: create(:company), ready_for_payment: true)
      end
    end

    describe ".pending" do
      it "returns only computations that are not ready for payment" do
        # Use the test company to ensure we only get the pending computation we created
        pending_computations = test_company.dividend_computations.pending
        expect(pending_computations).to contain_exactly(@pending_computation)
      end
    end
    
    describe ".ready_for_payment" do
      it "returns only computations that are ready for payment and have reached their issuance date" do
        # Should only include ready_computation, not future_computation
        ready_computations = test_company.dividend_computations.ready_for_payment
        expect(ready_computations).to contain_exactly(@ready_computation)
      end
    end
  end
  ...
  describe "#generate_dividends" do
    it "generates records as expected" do
      ...
      # Verify the dividend round was created
      dividend_round = @dividend_computation.dividend_rounds.first
      expect(dividend_round).to be_present
      expect(dividend_round.total_amount_in_usd).to eq(@dividend_computation.total_amount_in_usd)
      expect(dividend_round.issuance_date).to eq(@dividend_computation.dividends_issuance_date)
      expect(dividend_round.return_of_capital).to eq(@dividend_computation.return_of_capital)
      
      # Verify dividends were created for each output with a company_investor_id
      outputs_with_investor = @dividend_computation.dividend_computation_outputs.select { |o| o.company_investor_id.present? && o.total_amount_in_usd > 0 }
      
      outputs_with_investor.each do |output|
        dividend = company.dividends.find_by(company_investor_id: output.company_investor_id)
        expect(dividend).to be_present, "Expected to find a dividend for company_investor_id: #{output.company_investor_id}"
      end
      
      # Verify the dividend round was created
      expect(company.dividend_rounds.count).to eq(1)
